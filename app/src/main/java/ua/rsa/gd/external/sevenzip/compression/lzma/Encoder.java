package ua.rsa.gd.external.sevenzip.compression.lzma;

import ua.rsa.gd.external.sevenzip.ICodeProgress;
import ua.rsa.gd.external.sevenzip.compression.rangecoder.BitTreeEncoder;
...
public class Encoder
{
	...
	class LiteralEncoder
	{
		class Encoder2
		{
			...
			public void Init() { ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.InitBitModels(m_Encoders); }



			public void Encode(ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder rangeEncoder, byte symbol) throws IOException
			{
				int context = 1;
				for (int i = 7; i >= 0; i--)
				{
			...
			public void EncodeMatched(ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder rangeEncoder, byte matchByte, byte symbol) throws IOException
			{
				int context = 1;
				boolean same = true;
				for (int i = 7; i >= 0; i--)
			...
			public int GetPrice(boolean matchMode, byte matchByte, byte symbol)
			{
				...
				if (matchMode)
				{
					for (; i >= 0; i--)
					{
						int matchBit = (matchByte >> i) & 1;
						int bit = (symbol >> i) & 1;
						price += ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice(m_Encoders[((1 + matchBit) << 8) + context], bit);
						context = (context << 1) | bit;
						if (matchBit != bit)
						{
							i--;
				...
				{
					int bit = (symbol >> i) & 1;
					price += ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice(m_Encoders[context], bit);
					context = (context << 1) | bit;
				}
				return price;
			}
	...
	class LenEncoder
	{
		...
		{
			ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.InitBitModels(_choice);
		...
		public void Encode(ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder rangeEncoder, int symbol, int posState) throws IOException
		{
		...
		{
			int a0 = ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice0(_choice[0]);
			int a1 = ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice1(_choice[0]);
			int b0 = a1 + ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice0(_choice[1]);
			int b1 = a1 + ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice1(_choice[1]);
			int i = 0;
	...
	ua.rsa.gd.external.sevenzip.compression.lz.BinTree _matchFinder = null;
	ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder _rangeEncoder = new ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder();
	...
	void Create()
	{
		...
		{
			ua.rsa.gd.external.sevenzip.compression.lz.BinTree bt = new ua.rsa.gd.external.sevenzip.compression.lz.BinTree();
			int numHashBytes = 4;
	...
	void Init()
	{
		...
		ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.InitBitModels(_isMatch);
		ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.InitBitModels(_isRep0Long);
		ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.InitBitModels(_isRep);
		ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.InitBitModels(_isRepG0);
		ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.InitBitModels(_isRepG1);
		ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.InitBitModels(_isRepG2);
		ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.InitBitModels(_posEncoders);
	...
	{
		return ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice0(_isRepG0[state]) +
				ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice0(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
	}
	...
	int GetPureRepPrice(int repIndex, int state, int posState)
	{
		...
		if (repIndex == 0)
		{
			price = ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice0(_isRepG0[state]);
			price += ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice1(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
		}
		else
		{
			price = ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice1(_isRepG0[state]);
			if (repIndex == 1)
				price += ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice0(_isRepG1[state]);
			else
			{
				price += ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice1(_isRepG1[state]);
				price += ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice(_isRepG2[state], repIndex - 2);
			}
		}
		return price;
	}
	...
	int GetOptimum(int position) throws IOException
	{
		...
		_optimum[1].Price = ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice0(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]) +
				_literalEncoder.GetSubCoder(position, _previousByte).GetPrice(!Base.StateIsCharState(_state), matchByte, currentByte);
		_optimum[1].MakeAsChar();

		int matchPrice = ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice1(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]);
		int repMatchPrice = matchPrice + ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice1(_isRep[_state]);
		...
		int normalMatchPrice = matchPrice + ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice0(_isRep[_state]);
		...
		while (true)
		{
			...
			int curAnd1Price = curPrice +
				ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice0(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]) +
			...
			matchPrice = curPrice + ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice1(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]);
			repMatchPrice = matchPrice + ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice1(_isRep[state]);
			...
			if (!nextIsChar && matchByte != currentByte)
			{
				...
				if (lenTest2 >= 2)
				{
					...
					int nextRepMatchPrice = curAnd1Price +
						ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
						ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice1(_isRep[state2]);
					{
				...
				if (lenTest < numAvailableBytesFull)
				{
					...
					if (lenTest2 >= 2)
					{
						...
						int curAndLenCharPrice =
								...
								ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice0(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
						...
						int nextMatchPrice = curAndLenCharPrice + ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]);
						int nextRepMatchPrice = nextMatchPrice + ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice1(_isRep[state2]);
			...
			if (newLen >= startLen)
			{
				normalMatchPrice = matchPrice + ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.GetPrice0(_isRep[state]);
					...
					if (lenTest == _matchDistances[offs])
					{
						if (lenTest < numAvailableBytesFull)
						{
							...
							if (lenTest2 >= 2)
							{
								...
								state2 = Base.StateUpdateChar(state2);
								posStateNext = (position + lenTest + 1) & _posStateMask;
								int nextMatchPrice = curAndLenCharPrice + ua.rsa.gd.external
	...
	void FillDistancesPrices()
	{
		...
		for (int lenToPosState = 0; lenToPosState < Base.kNumLenToPosStates; lenToPosState++)
		{
			...
			for (posSlot = Base.kEndPosModelIndex; posSlot < _distTableSize; posSlot++)
				_posSlotPrices[st + posSlot] += ((((posSlot >> 1) - 1) - Base.kNumAlignBits) << ua.rsa.gd.external.sevenzip.compression.rangecoder.Encoder.kNumBitPriceShiftBits);
